...
#define ASSERT(...)
#else
#define INTERNAL_ASSERT_IMPL(type, check, msg, ...) {if(!(check)) {ERROR(msg,__VA_ARGS__); __debugbreak(); }}
#define INTERNAL_ASSERT_MSG(type, check, msg, ...) INTERNAL_ASSERT_IMPL(type, check, "Assertion failed: {0}", __VA_ARGS__)
